...
@section('title', 'Cart Pengambilan')
...
<section class="section">
  ...
  <div class="section-header">
    ...
    <h1 class=" d-xl-none d-md-none">Cart Pengambilan</h1>
    <div class="section-header-breadcrumb ml-md-0 d-none d-md-flex">
      ...
      <div class="breadcrumb-item">Cart Pengambilan</div>
    </div>
  </div>
  ...
  <div class="section-body">
    <h2 class="section-title text-capitalize">pengambilan barang</h2>
    <p class="section-lead">Pemrosesan pengambilan barang untuk user.</p>
    @if ( !empty( $data ) )
        @foreach ( $data as $d )
        <div class="row">
          <div class="col-md-12">
            <div class="card">
                      ...
                      <div class="product-details">
                        ...
                        <div class="product-cta">
                          <span class="btn btn-danger" onclick="$(this).find('form').submit()">
                            Hapus
                            <form action="{{ route('cart_ambil_remove') }}" method="post">
                              @csrf
                              @method('delete')
              ...
              <div class="card-footer text-right">
                <span class="btn btn-danger" onclick="$(this).find('form').submit()">
                  Batal
                  <form action="{{ route('cart_ambil_destroy', ['id' => $d->user_id]) }}" method="post">
                    @csrf
                    @method('delete')
                  </form>
                </span>
                <span class="btn btn-primary" onclick="$(this).find('form').submit()">
                  Proses
                  <form action="{{ route('cart_ambil_store') }}" method="post">
                    @csrf
                    @method('put')
                    <input type="hidden" name="user" value="{{ $d->user_id }}">
                  </form>
            ...
            <div class="card">
              ...
              <div class="card-body">
                <div class="elemet-carousel owl-carousel owl-theme">
                  ...
                  @foreach ( Cart::session("ambil_$u")->getContent() as $item )
                  <div>
                    <div class="product-item pb-3">
                      <div class="product-image">
                        ...
                        <div class="product-cta">
                          <span class="btn btn-danger" onclick="$(this).find('form').submit()">
                            ...
                            <form action="{{ route('cart_remove') }}" method="post">
                              ...
                              <input type="hidden" name="item" value="{{ $item->id }}">
                              <input type="hidden" name="type" value="{{ 'ambil' }}">
                            </form>
                          </span>
                        </div>
                      </div>
