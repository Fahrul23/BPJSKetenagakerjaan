...
<section class="section">
  ...
  <div class="section-header">
    ...
    <h1 class=" d-xl-none d-md-none">Laporan Pengambilan</h1>
    <div class="section-header-breadcrumb ml-md-0 d-none d-md-flex">
      <div class="breadcrumb-item active"><a href="{{url('/')}}">Home</a></div>
      <div class="breadcrumb-item">Laporan Pengambilan</div>
    </div>
  </div>
  ...
  <div class="section-body">
    <div class="row">
      <div class="col-12">
        <form action="{{route('laporan_pengambilan')}}" method="GET">
          <div class="card">
    ...
    <div class="row">
      <div class="col-12 col-md-12">
        <div class="card">
          <div class="card-header">
            ...
            <div class="card-header-action">
              <div class="dropdown">
                ...
                <form action="{{route('laporan_pengambilan_export_excel')}}" method="POST">
                  @csrf
                  {{-- <input type="hidden" name="category_id" value="{{request()->get('category')}}"> --}}
                  {{-- <input type="hidden" name="item_name" value="{{request()->get('item')}}"> --}}
                  ...
                  <a class="dropdown-item has-icon" href="#" onclick="$(this).find('form').submit()"><i class="fas fa-file-pdf" style="color: #fc544b;"></i> Export PDF
                    <form action="{{route('laporan_pengambilan_export_pdf')}}" method="POST">
                  @csrf
                  {{-- <input type="hidden" name="category_id" value="{{request()->get('category')}}"> --}}
                  {{-- <input type="hidden" name="item_name" value="{{request()->get('item')}}"> --}}
                  <input type="hidden" name="user_name" value="{{request()->get('user')}}">
              ...
              <table class="table table-striped table-1 text-center">
                ...
                <tbody>
                  ...
                  <tr>
                    ...
                    <td>{{$pengambilan->user->name}}</td>
                    <td>{{$pengambilan->created_at->isoFormat('DD MMMM Y')}}</td>
                    <td>
                      <a href="{{route('laporan_pengambilan_detail', ['id' => $pengambilan->id])}}" class="btn btn-primary">Detail</a>
                    </td>
                  </tr>
                  @empty
                    <tr>
          ...
          @if ( $pengambilans->hasPages() )
            <div class="card-footer">
                {{ 
                    $pengambilans->appends([
